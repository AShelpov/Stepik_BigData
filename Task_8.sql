...
/*насколько я понял нужно выявить объявления по которым общий массив просмотров наступил позднее чем самый
  первый клик. Поскольку у нас нет идентификатора пользователя другого решения задачи я пока не вижу.
  Таким образом в рамках решения такой задачи нужно взять отсортированный массив по показам для каждого объявления
  и отсортированный массив по кликам для каждого объявления. Из этих двух массивов взять самый первый клик
  и самый первый просмотр и если время первого клика будет меньше времени первого просмотра, то получается
  что мы выявили именно такое объявление*/
